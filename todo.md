...
## 自サーバーでの/operatorページ404問題（新規）
- [ ] サーバー側のルーティング設定を調査
- [ ] SPAフォールバック設定を確認・修正
- [ ] 修正版をユーザーに提供

- [x] オペレーターページが一瞬表示されてから404になる問題を修正（デモ版として認証不要に変更）


## QRコード読み取りバグ修正（新規）
- [ ] 異なるスペースのQRコードを読み取っても同じスペースの決済ページになるバグを調査
- [ ] バグを修正
- [ ] テスト・検証


## オーナーIDで直接アクセスできるルート追加（新規）
- [ ] /owner/{openId}ルートを追加
- [ ] ownerProcedureをopenIdパラメータ対応に修正
- [ ] テスト・検証


## オーナーマイページのプロフィール編集バグ修正（新規）
- [ ] どのオーナーのマイページからプロフィールを変更しても、demo-userが変更される問題を修正
- [ ] OwnerDashboardのProfileTabでopenIdを使用するように修正
- [ ] owner.updateProfileプロシージャでopenIdを受け取るように修正
- [ ] テスト・検証

## 本番版（parking-system-prod）決済機能追加
- [x] オペレーターページに決済設定セクションを追加（PayPay/LINE Pay/楽天ペイ/Stripe APIキー設定）
- [ ] 決済画面に複数決済方法を追加（PayPay/LINE Pay/楽天ペイ/クレジットカード）
- [ ] 認証を有効化（デモモード解除）
- [ ] ポート3001で起動するように設定
- [ ] 本番サーバーにデプロイ
