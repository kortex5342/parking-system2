...
## 複数の時間帯設定機能（新規）
- [x] ParkingLotDetailDialogの時間帯ごとの最大料金セクションを拡張
- [x] 複数の時間帯を追加・削除できるテーブルUIを実装
- [x] 時間帯の追加フォームを実装
- [x] バックエンドAPIで複数の時間帯を保存・更新
- [x] テスト・検証

## 複数の時間帯を跨ぐ料金計算ロジック（新規）
- [ ] 複数の時間帯を跨ぐ駐車の料金計算ロジックを実装
- [ ] 出庫API（calculateFee）に複数の時間帯対応を追加
- [ ] 料金計算の正確性を検証するテストを追加
